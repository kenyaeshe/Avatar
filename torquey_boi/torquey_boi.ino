...
BLDCMotor motor = BLDCMotor(-1, 3.65);  // Inits the motor. The argument is the number of pole pairs physically in the motor; the EC-i 539482 has 4, the EC-max 272768 has 1
BLDCDriver3PWM driver = BLDCDriver3PWM(9, 5, 6, 8);  // Inits the PWM and enable channels for the motor.
...
void setup() {
  ...
  // foc currnet control parameters (stm/esp/due/teensy)
   ...
   motor.current_limit = 1; // Amps WOW I ACTUALLY WROTE A LINE OF CODE!!! Actually, I don't think this line of code does anything??
...
void loop() {
  ...
  if (sensor.getAngle() < 2.0)
  {
    // Serial.print("leq!");
    com_current = -100*(2.0-sensor.getAngle());
  } else {
    com_current = 0;
  }
////  Serial.print(sensor.getAngle());
////  Serial.println(com_current);
  motor.move(com_current );

  // user communication
  command.run();

  // Idk if it will let me take inputs and send outputs over Serial at the same time but here goes
