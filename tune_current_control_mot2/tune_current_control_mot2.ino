...
InlineCurrentSense current_sense = InlineCurrentSense(0.01, 50.0, MOT2_CS_A, MOT2_CS_B);  // Instantiates on-board current sensing
...
BLDCDriver3PWM driver = BLDCDriver3PWM(MOT2_PWM_A, MOT2_PWM_B, MOT2_PWM_C, MOT2_ENABLE);  // Inits the PWM and enable channels for the motor. Arguments are pin numbers on the FOC shield. Do not change.
float last_vel = 0;
...
void loop() {
  ...
  motor.loopFOC();
  if (ctr > 30) {
    Serial.print(com_current); 
    Serial.print(" "); 
    Serial.print(current_sense.getFOCCurrents(motor.electricalAngle()).q); 
    Serial.print(" ");
    Serial.print(current_sense.getDCCurrent(motor.electricalAngle())); 
    Serial.print(" ");  
    Serial.println(current_sense.getFOCCurrents(motor.electricalAngle()).d);
    ctr = 0;
  } 
